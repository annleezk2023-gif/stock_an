...
# 查询除权除息数据
def query_dividend_data(fund_code, fund_name, year):
    """
    查询单只基金的持仓数据
    :param fund_code: 基金代码
    :param fund_name: 基金名称
    :param year: 年份
    :return: 持仓数据的DataFrame
    """
    if year is None:
        return
    logger.debug(f"查询 {fund_name}({fund_code}) 的持仓数据: {year}")
    
    # 使用akshare的API查询持仓数据
    rs = ak.fund_portfolio_hold_em(symbol=fund_code, date=str(year))
    ...
    if not data_list:
        logger.debug(f"没有找到 {fund_name}({fund_code}) 的持仓数据")
        return None
...
# 批量获取并保存除权除息数据
def batch_fetch_and_save_dividend_data(start_year=2007, conn=None):
    """
    ...
    for index, stock in enumerate(stocks):
        ...
        for year in range(start_year, end_year + 1):
            ...
            if result[0] > 0:
                logger.info(f"{index+1}/{total_stocks}  股票 {stock.code} {stock.code_name} {year}年 除权除息数据已存在，跳过")
                continue
            ...
            if dividend_data is not None and not dividend_data.empty:
                ...
                success_count += saved
                logger.info(f"{index+1}/{total_stocks}  成功保存{saved}条除权除息数据")
            else:
                save_empty_dividend_data(stock.code, year, conn)
                logger.info(f"{index+1}/{total_stocks} 股票 {stock.code} {stock.code_name} {year}年 除权除息数据为空，保存空数据成功")
...
if __name__ == "__main__":
    ...
    try:
        ...
        batch_fetch_and_save_dividend_data(start_year=2007, conn=conn)
        conn.close()
