...
# 获取单个非股票的K线数据
def fetch_single_nostock_kline(stock_code, stock_name, start_date, end_date, adjustflag=1):
    """获取单个非股票的K线数据"""
...
# 批量获取并保存非股票K线数据
def batch_fetch_and_save_kline_data(conn=None):
    date_2007 = date(2007, 1, 1)
    end_date = datetime.now().strftime('%Y-%m-%d')

    # 获取所有非股票代码
    nostocks = stock_common.get_nostock_info_all(conn)
    
    if not nostocks:
        logger.info("未获取到非股票数据")
        return
    
    total_nostocks = len(nostocks)
    logger.info(f"开始处理{total_nostocks}只非股票的K线数据")

    # 处理每只非股票
    for index, stock in enumerate(nostocks):
        ...
        """
        # 查询表中最新日期
        result = conn.execute(text("SELECT MAX(date) FROM bao_nostock_trade WHERE code = :code"), 
            {'code': stock_code}
        ).fetchone()
        ...
        logger.info(f"处理第{index + 1}/{total_nostocks}只非股票: {stock_name}({stock_code})  {start_date}  {end_date}")

        if start_date == date_2007:
            logger.info(f"  非股票 {stock_name}({stock_code}) 没有数据，跳过")
            continue

        # 如果start_date>昨天，就跳过当前股票。即最新数据己拿到
        yesterday = datetime.now().date() - timedelta(days=1)
        if start_date > yesterday:
            logger.debug(f"  非股票 {stock_name}({stock_code}) 最新数据已获取，跳过")
            continue
    
    
        if "0" == stock.status and stock.out_date is not None and stock.out_date.strftime('%Y-%m-%d') == latest_date.__str__():
            logger.debug(f"{index+1}/{total_nostocks}  非股票 {stock_name}({stock_code}) 退市前全部数据已获取，跳过")
            continue
        ...
        if tradeday_num == 0:
            logger.debug(f"{index+1}/{total_nostocks}  非股票 {stock_name}({stock_code}) {start_date} {end_date} 无交易日，跳过")
            continue

        # 获取K线数据
        df = fetch_single_nostock_kline(stock_code, stock_name, start_date.__str__(), end_date.__str__())
        time.sleep(0.1)
        
        if df is not None and not df.empty:
            # 保存数据到表
            saved_count = save_kline_data(df, conn)
            logger.debug(f"  成功保存{saved_count}条数据")
        else:
            logger.debug(f"  没有获取到数据或数据为空")

        if index % 100 == 0:
            time.sleep(1)
        
    logger.info("所有非股票K线数据处理完成！")


if __name__ == "__main__":
    logger.info("开始获取非股票历史K线数据...")
