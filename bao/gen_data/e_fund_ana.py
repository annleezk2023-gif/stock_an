...
def get_industry_stocks(conn, industry):
    sql = f"""SELECT code FROM bao_stock_basic WHERE industry = :industry AND status = '1' ORDER BY code ASC"""
...
def fund_ana_data_gen():
    ...
    try:
        ...
        for industry_row in industries:
            ...
            # 获取该行业的所有股票代码
            codes = get_industry_stocks(conn, industry)
            ...
            logger.info(f"行业 {industry} 共有 {len(codes)} 只股票")
            ...
            upsert_sql = """INSERT INTO fund_ana (industry, all_stock, top_stock, pe_year_1_percent, pe_year_3_percent, pe_year_5_percent, pe_year_10_percent, created_at, updated_at)
                            VALUES (:industry, :all_stock, :top_stock, :pe_year_1_percent, :pe_year_3_percent, :pe_year_5_percent, :pe_year_10_percent, NOW(), NOW())
